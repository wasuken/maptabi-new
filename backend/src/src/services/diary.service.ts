...
export const getDiaryById = async (diaryId: number, userId: number): Promise<DiaryWithLocations | null> => {
  const client = await pool.connect();
...
export const updateDiary = async (diaryId: number, diaryInput: DiaryInput, userId: number): Promise<Diary> => {
  const client = await pool.connect();
...
export const deleteDiary = async (diaryId: number, userId: number): Promise<void> => {
  ...
  try {
    ...
    // 関連する位置情報を削除
    await client.query(
      'DELETE FROM locations WHERE diary_id = $1',
      [diaryId]
    );
...
export const getDiaryLocations = async (diaryId: number, userId: number): Promise<DiaryLocation[]> => {
  const client = await pool.connect();
