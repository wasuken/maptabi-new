...
export const login = async (loginInput: LoginInput): Promise<AuthResponse> => {
  ...
  try {
    // ユーザーの取得
    const result = await client.query('SELECT * FROM users WHERE email = $1', [loginInput.email]);
    ...
    // パスワードの検証
    const isPasswordValid = await comparePassword(loginInput.password, user.password_hash);
